...
impl<T: 'static + Number, U: 'static + Number> Cakes<T, U> {
    ...
    pub fn build_in_batches(
        ...
        for (i, batch_indices) in complement_indices.chunks(batch_size).enumerate() {
            ...
            let insertion_paths: Vec<Vec<_>> = batch_indices
                ...
                .enumerate()
                .map(|(i, _)| {
                    let instance = batch_dataset.get(i);
    ...
    fn query_distance(&self, query: &[T], index: Index) -> U {
        self.distance(query, &self.dataset.get(index))
    }
}
...
mod tests {
    ...
    fn test_search() {
        ...
        let dataset: Arc<dyn Dataset<f64, f64>> = Arc::new(Tabular::new(Arc::new(data), metric, false));
        ...
        let query = Arc::new(search.dataset.get(1));
    ...
    fn test_search_large() {
        ...
        let metric = metric_from_name("euclidean").unwrap();
        let dataset: Arc<dyn Dataset<_, f64>> = Arc::new(Tabular::new(Arc::new(data), metric, true));
