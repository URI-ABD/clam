...
pub type Candidates<T, U> =
    HashMap<Arc<Cluster<T, U>>, HashMap<Arc<Cluster<T, U>>, U>>;
...
impl<T: Number, U: Number> Manifold<T, U> {
    pub fn new(
        ...
        partition_criteria: &[PartitionCriterion<T, U>],
    ) -> Arc<Self> {
    ...
    fn compute_candidates(&self) -> Candidates<T, U> {
        ...
        while !parents.is_empty() {
            let child_candidates: Candidates<T, U> = parents
                .par_iter()
                .flat_map(|parent| {
                    match parent.children.read().unwrap().clone() {
                        Some((left, right)) => {
                            ...
                            vec![
                                (
                                    Arc::clone(&left),
                                    left.find_candidates(parent_candidates),
                                ),
                                (
                                    Arc::clone(&right),
                                    right.find_candidates(parent_candidates),
                                ),
                            ]
                        }
                        None => Vec::new(),
    ...
    fn create_graph(&self, clusters: &[Arc<Cluster<T, U>>]) -> Graph<T, U> {
        let edges = clusters
            .par_iter()
            .flat_map(|cluster| {
    ...
    pub fn create_layer_graphs(&self) -> Vec<Arc<Graph<T, U>>> {
        ...
        while !tree.is_empty() {
            let (mut layer, rest): (Vec<_>, Vec<_>) = tree
                .into_par_iter()
                .partition(|cluster| cluster.depth() == depth);
            tree = rest;
            depth += 1;
            ...
            let mut new_leaves = layer
                .par_iter()
                .filter(|&cluster| {
                    cluster.children.read().unwrap().clone().is_none()
                })
                .cloned()
                .collect();
    ...
    fn select_clusters(
        ...
        let mut scores_heap: ScoresHeap<T, U> = clusters
            .into_iter()
            .chain(
                shallow_clusters.into_iter().filter(|cluster| {
                    cluster.children.read().unwrap().is_none()
                }),
            )
            .map(|cluster| {
                (
                    OrderedFloat::from_f64(criterion(cluster.ratios)).unwrap(),
                    cluster,
                )
            })
            .collect();
        ...
        while !scores_heap.is_empty() {
            ...
            scores_heap = scores_heap
                .into_par_iter()
                .filter(|(_, other)| {
                    !other.is_ancestor_of(&cluster)
                        && !cluster.is_ancestor_of(other)
                })
                .collect();
    ...
    pub fn create_optimal_graphs(
        ...
        meta_ml_scorers
            .iter()
            .map(|criterion| {
                self.select_clusters(criterion, min_selection_depth)
            })
            .map(|clusters| self.create_graph(&clusters))
            .map(Arc::new)
            .collect()
    }
    ...
    pub fn ancestry(
        ...
        for &go_right in name.iter().by_ref() {
            let (left, right) =
                match ancestors.last().unwrap().children.read().unwrap().clone()
                {
                    Some((left, right)) => (left, right),
                    None => {
                        return Err(format!(
                            "cluster {:?} not found in the tree",
                            name
                        ))
                    }
                };
