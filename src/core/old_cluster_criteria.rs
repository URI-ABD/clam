...
pub trait PartitionCriterion<T: Number>: std::fmt::Debug + Send + Sync {
    fn check(&self, c: &Cluster<T>) -> bool;
}

#[derive(Debug)]
pub struct PartitionCriteria<T: Number> {
    criteria: Vec<Box<dyn PartitionCriterion<T>>>,
    check_all: bool,
}

impl<T: Number> PartitionCriteria<T> {
    ...
    pub fn with_custom(mut self, c: Box<dyn PartitionCriterion<T>>) -> Self {
        self.criteria.push(c);
        self
    }

    pub fn check(&self, cluster: &Cluster<T>) -> bool {
...
impl<T: Number> PartitionCriterion<T> for MaxDepth {
    fn check(&self, c: &Cluster<T>) -> bool {
        c.depth() < self.0
    }
}
...
impl<T: Number> PartitionCriterion<T> for MinCardinality {
    fn check(&self, c: &Cluster<T>) -> bool {
