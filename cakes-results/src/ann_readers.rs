...
pub fn read_search_data(name: &str) -> Result<TrainTest<f32>, String> {
    ...
    let train_data = read_npy(&make_path_npy(&data_dir, name, "train"))?;

    let test_data = read_npy(&make_path_npy(&data_dir, name, "test"))?;

    Ok((train_data, test_data))
}

fn make_path_npy(dir: &std::path::Path, name: &str, variant: &str) -> std::path::PathBuf {
