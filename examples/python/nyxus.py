...
class NyxusDataset(csv_space.CsvDataset):
    def __init__(
        ...
        *,
        ...
        assert (
            file_names_column in data.names
        ), f"Column {file_names_column} not found in the dataset."

        file_names: list[str] = data[file_names_column].to_list()[0]
        pattern = re.compile(
            "(?P<d>\\w+)_p(?P<p>\\d+)_y(?P<y>\\d+)_r(?P<r>\\d+)_c(?P<c>\\d+).tif",
        )
...
def experiment():
    ...
    nyxus_train = nyxus_full.subset(
        train_indices,
        f"{nyxus_full.name}_train",
        labels=train_labels,
    )
    nyxus_test = nyxus_full.subset(
        test_indices,
        f"{nyxus_full.name}_test",
        labels=test_labels,
    )
