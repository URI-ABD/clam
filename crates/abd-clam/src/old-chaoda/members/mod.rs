...
impl Member {
    ...
    pub fn evaluate_clusters<U: Number, C: OddBall<U>>(&self, g: &mut Graph<U, C>) -> Vec<f32> {
        match self {
    ...
    pub fn evaluate_points<U: Number, C: OddBall<U>>(&self, g: &mut Graph<U, C>) -> Vec<f32> {
        match self {
...
trait Algorithm: Default + Clone + Send + Sync + Serialize + for<'de> Deserialize<'de> {
    ...
    fn inherit_scores<U: Number, C: OddBall<U>>(&self, g: &Graph<U, C>, scores: &[f32]) -> Vec<f32> {
    ...
    ///
    /// * A vector of anomaly scores for each point in the `Graph`.
    fn evaluate_points<U: Number, C: OddBall<U>>(&self, g: &mut Graph<U, C>) -> Vec<f32> {
        let cluster_scores = {
