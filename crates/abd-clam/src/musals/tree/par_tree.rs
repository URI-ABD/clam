//! Parallel version of the `Tree` struct with methods for performing multiple sequence alignment using the MUSALS algorithm.
...
{
    /// Parallel version of [`align_bottom_up`](Self::align_bottom_up).
    pub(crate) fn par_align_bottom_up(&mut self, cost_matrix: &CostMatrix<T>) {
        ...
        let mut frontier = leaves
            .into_keys()
            .inspect(|&id| {
                self.par_align_leaf(id, cost_matrix);
        ...
        while !frontier.is_empty() {
            ...
            frontier = full_parents
                .into_keys()
                .inspect(|&id| {
                    self.par_align_parent(id, cost_matrix);
                    if let Some(waiting) = self.items[id]
                        .2
                        .as_cluster()
                        .and_then(|c| c.parent_center_index)
    ...
    /// Parallel version of [`align_leaf`](Self::align_leaf).
    fn par_align_leaf(&mut self, id: usize, cost_matrix: &CostMatrix<T>) {
        ...
        for i in 1..car {
            ...
            self.par_insert_gaps(&c_gaps, id..id + i);
            self.items[id + i].1.insert_gaps(&t_gaps);
        }
    }

    /// Parallel version of [`align_parent`](Self::align_parent).
    fn par_align_parent(&mut self, id: usize, cost_matrix: &CostMatrix<T>) {
        ...
        for &r_id in &cids[1..] {
            ...
            self.par_insert_gaps(&l_gaps, l_id..l_id + aligned_car);
            ...
            self.par_insert_gaps(&r_gaps, r_id..r_id + r_car);
            aligned_car += r_car;
        }
        ...
        self.par_insert_gaps(&l_gaps, id..id + 1);
        self.par_insert_gaps(&r_gaps, (id + 1)..(id + 1 + aligned_car));
    }

    /// Parallel version of [`insert_gaps`](Self::insert_gaps).
    fn par_insert_gaps(&mut self, gap_indices: &[usize], seq_range: core::ops::Range<usize>) {
        self.items[seq_range].par_iter_mut().for_each(|(_, seq, _)| seq.insert_gaps(gap_indices));
    }
}
