//! Trees built under Cosine distance.
...
/// Trees for Vector data under Cosine distance.
#[derive(Debug, Clone)]
#[non_exhaustive]
pub enum CosineTree {
...
impl From<CosineTree> for ShellTree {
    fn from(cosine_tree: CosineTree) -> Self {
        Self::Cosine(cosine_tree)
    }
}

impl databuf::Encode for CosineTree {
...
impl<'de> databuf::Decode<'de> for CosineTree {
    fn decode<const CONFIG: u16>(buffer: &mut &'de [u8]) -> databuf::Result<Self> {
        ...
        match &variant {
            b"F64" => {
                ...
                Ok(Self::F64(tree.with_metric(common_metrics::cosine)))
            }
            b"F32" => {
                let tree = Tree::decode::<CONFIG>(buffer)?;
                Ok(Self::F32(tree.with_metric(common_metrics::cosine)))
            }
            _ => Err(format!("Invalid variant for CosineTree: {variant:?}. Expected one of: F64, F32").into()),
        }
    }
}
...
impl CosineTree {
    ...
    /// * If the data type is not compatible with the Cosine metric.
    pub fn build<P: AsRef<std::path::Path>, R: rand::Rng>(
        ...
        match data_type {
            ShellDataType::F64 => {
                ...
                let metric: fn(&_, &_) -> f64 = common_metrics::cosine;
            ...
            ShellDataType::F32 => {
                ...
                let data = data.into_iter().enumerate().collect(); // Convert to (index, vector) pairs.
                let data = shuffle_and_truncate(data, rng, num_samples);
                let metric: fn(&_, &_) -> f32 = common_metrics::cosine;
                let tree = Tree::par_new(data, metric, &|_| (), &|c| c.cardinality() > 2, strategy)?;
                Ok(Self::F32(tree))
            }
            _ => Err(format!("Invalid data type for building a tree with the Cosine metric: {data_type:?}. Expected one of: 'F64', 'F32'").into()),
        }
    }
