...
impl Group {
    ...
    pub fn new(path_manager: PathManager, uncompressed: Co, queries: Vec<(M, I)>) -> Result<Self, String> {
        ...
        if !query_path.exists() {
            ...
            std::fs::write(path_manager.queries_path(), &bytes).map_err(|e| e.to_string())?;
        }
    ...
    where
        Aco: ParSearchAlgorithm<I, U, B, Hamming, Co>,
        Adec: ParSearchAlgorithm<I, U, Sb, Hamming, Dec>,
    {
        let metric = &Hamming;
        let name = alg_a.name();
